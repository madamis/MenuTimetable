@extends('layouts.app')
@section('content')
    <form class="p-3 my-4 rounded-md bg-white" method="post" action="/foodcategories" autocomplete="off">
        @csrf
                ...
                <tbody>
                    @foreach ($foodcategories as $foodcategory)
                        <tr class="bg-white odd:bg-gray-200 border-b dark:bg-gray-800 dark:border-gray-700">
                            ...
                            <td>
                                <button @click="showEditModal({{ $foodcategory->id }})"
                                    class="bg-blue-500 text-white px-2 py-1 rounded-md"> Edit
                                </button>
                                <button @click="showDeleteModal({{ $foodcategory->id }})"
        ...
        <div x-clock x-show="editOpen" class="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title"
            ...
            <div class="flex items-end justify-center min-h-screen px-4 text-center md:items-center sm:block sm:p-0">
                ...
                <div x-cloak x-show="editOpen" x-transition:enter="transition ease-out duration-300 transform"
                    ...
                    <form class="mt-5" :action="'foodcategories/' + foodcategory_id" method="post">
                        @csrf
                        @method('put')
                        <div>
                            <label for="user name" class="block text-sm text-gray-700 capitalize dark:text-gray-200">Short
                                name</label>
                            <input x-model="foodcategory.name" name="name" placeholder="Name" type="text"
                        ...
                        <div class="mt-4">
                            <label for="email" class="block text-sm text-gray-700 capitalize dark:text-gray-200">Full
                                name</label>
                            <input x-model="foodcategory.description" name="description" placeholder="Description"
                                type="text"
                                class="block w-full px-3 py-2 mt-2 text-gray-600 placeholder-gray-400 bg-white border border-gray-200 rounded-md focus:border-indigo-400 focus:outline-none focus:ring focus:ring-indigo-300 focus:ring-opacity-40">
        ...
        <div x-clock x-show="openDelete" class="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title"
            ...
            <div class="flex items-end justify-center min-h-screen px-4 text-center md:items-center sm:block sm:p-0">
                ...
                <div x-cloak x-show="openDelete" x-transition:enter="transition ease-out duration-300 transform"
                    ...
                    class="inline-block w-full max-w-xl p-8 my-20 overflow-hidden text-left transition-all transform bg-white rounded-lg shadow-xl 2xl:max-w-2xl">
                    <div class="flex items-center justify-between space-x-4">
                        <h1 class="text-xl font-medium text-gray-800 text-center">Delete foodcategory</h1>
                    ...
                    <div class="p-6 text-center">
                        ...
                        <h3 class="mb-5 text-lg font-normal text-red-500 dark:text-gray-400">Are you sure you want to
                            delete
                            <b><span x-text="foodcategory.name"></span>?</b>
                        </h3>
                        <form :action="'/foodcategories/' + foodcategory_id" method="post">
                            @csrf
                            @method('delete')
                            <button type="submit"
    ...
    <script>
        document.addEventListener('alpine:init', () => {
            Alpine.data('menuSystem', () => ({
                ...
                async getfoodcategory(id) {
                    this.foodcategory_id = id;
                    await fetch(`/foodcategories/${this.foodcategory_id}`)
                        .then((response) => response.json())
                        .then((data) => this.foodcategory = data);
                },

                showEditModal(id) {
                    this.getfoodcategory(id)
                    // set values 
                    this.toggleEditOpen()
                },
                ...
                emptyDescription() {
                    return this.foodcategory.description === ""
                },
